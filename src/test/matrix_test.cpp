...
TEST(OtherMatrixOperations, VerifyIdentiyMatrices){
    ...
    Matrix m2(v2);
    Matrix m3 = Matrix::identiyMatrix(3);
    EXPECT_EQ(m1.is_identity(), false);
    EXPECT_EQ(m2.is_identity(), true);
    EXPECT_EQ(m3.is_identity(), true);
...
TEST(OtherMatrixOperations, VerifyInverseMatrices){
    ...
    Matrix m2(v2);
    Matrix m3(v3);
    EXPECT_EQ(m1.is_inverse(m2), false);
    EXPECT_EQ(m1.is_inverse(m3), true);
}
